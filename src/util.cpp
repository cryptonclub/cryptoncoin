...
const char * const BITCOIN_CONF_FILENAME = "cryptoncoin.conf";
const char * const BITCOIN_PID_FILENAME = "cryptoncoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "cryptoncoin" is a composite category enabling all Cryptoncoin-related debug output
                if(ptrCategory->count(std::string("cryptoncoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Cryptoncoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Cryptoncoin
    // Mac: ~/Library/Application Support/Cryptoncoin
    // Unix: ~/.cryptoncoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cryptoncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cryptoncoin";
#else
    // Unix
    return pathRet / ".cryptoncoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty cryptoncoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override cryptoncoin.conf
