...
# cryptoncoin_hash
RUN git clone https://github.com/cryptoncoinpay/cryptoncoin_hash
RUN cd cryptoncoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} cryptoncoin
RUN useradd -u ${USER_ID} -g cryptoncoin -s /bin/bash -m -d /cryptoncoin cryptoncoin
...
RUN mkdir /cryptoncoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /cryptoncoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /cryptoncoin-src
