...
export BUILDER_IMAGE_NAME="cryptoncoin-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/cryptoncoin"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_CCN_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
